...
const match =
    ((c.firstName || "") + " " + (c.lastName || "")).toLowerCase().includes(term) || // full name
    ...
    (c.vehicles || []).some(v => (v.rego || "").toLowerCase().includes(term));       // vehicle rego
...
/* Render vehicles under customer only once */
